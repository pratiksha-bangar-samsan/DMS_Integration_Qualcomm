...
class EdgetensorDMMMonitor:
    def __init__(self, roi_position, config_file='config.ini'):
        ...
        self.audio_path = "/home/jetson/Desktop/audio_alert" # New Audio Path
        self.icons = {}
        self.load_icons()
        ...
        self.voice_alerts = {
            'yawning': 'yawning.mp3',
            'fatigue': 'fatigue_HoldSteering.mp3',
        ...
        if self.can_enabled:
            try:
                # Configuration for SocketCAN on Linux, using the required ID 0x307
    ...
    def send_can_message(self, message_data):
        ...
        try:
            msg = can.Message(
                ...
                is_extended_id=False # Standard CAN ID (11-bit)
            )
            self.can_bus.send(msg)
        except Exception as e:
            # print(f"Error sending CAN message ID {hex(message_data['id'])}: {e}")
            pass
