...
import paho.mqtt.client as mqtt # <--- ADDED for MQTT communication
...
class DMSMonitor:
    def __init__(self, roi_position, config_file='config.ini'):
        ...
        # *** REMEMBER TO REPLACE THIS WITH YOUR ACTUAL TOKEN ***
        self.TB_ACCESS_TOKEN = "A5k1VSyai6BjNUkRHfjL" 
        self.TB_BROKER = "thingsboard.cloud"
        self.TB_PORT = 1883
...
# --- MQTT Initialization is now called in __init__ ---
...
# -----------------------------------------------------
    ...
    def on_mqtt_connect(self, client, userdata, flags, rc):
        if rc == 0:
            print(f"✅ MQTT: Connected successfully to ThingsBoard.")
        else:
            print(f"❌ MQTT: Connection failed. Return code: {rc}")

    def initialize_mqtt(self):
        """Sets up and starts the MQTT client connection to ThingsBoard."""
        self.mqtt_client = mqtt.Client()
        self.mqtt_client.username_pw_set(self.TB_ACCESS_TOKEN)
        self.mqtt_client.on_connect = self.on_mqtt_connect
        
        try:
            self.mqtt_client.connect(self.TB_BROKER, self.TB_PORT, 60)
            self.mqtt_client.loop_start() # Start non-blocking loop
        except Exception as e:
            print(f"Fatal error during MQTT connection setup: {e}")
            self.mqtt_client = None

    def publish_mqtt_telemetry(self):
        """
        Creates the binary (1/0) JSON payload from active events 
        and publishes it if the state has changed.
        """
        
        # 1. Generate the current state (1 for active, 0 for inactive)
        current_state = {
            'yawning': 1 if self.events['yawning']['active'] else 0,
            'fatigue': 1 if self.events['fatigue']['active'] else 0,
            'distraction': 1 if self.events['distraction']['active'] else 0,
            'phone_uses': 1 if self.events['phone_uses']['active'] else 0,
            'smoking': 1 if self.events['smoking']['active'] else 0,
            'eating_drinking': 1 if self.events['eating_drinking']['active'] else 0,
            'face_obstruction': 1 if self.events['face_obstruction']['active'] else 0,
            'camera_obstruction': 1 if self.events['camera_obstruction']['active'] else 0,
            'emergency_alert': 1 if self.events['emergency_alert']['active'] else 0,
            # Continuous metrics are useful for dashboards but don't require 1/0
            'head_yaw': round(self.head_pose_yaw, 1),
            'head_pitch': round(self.head_pose_pitch, 1),
            'eyes_closed_time_s': round(max(self.events['left_eye_closed']['timer'], self.events['right_eye_closed']['timer']), 1)
        }
        
        # 2. Check for state change
        if self.mqtt_client and current_state != self.last_published_state:
            json_payload = json.dumps(current_state)
            
            # 3. Publish
            result = self.mqtt_client.publish(self.TB_TELEMETRY_TOPIC, json_payload, qos=1) # QoS 1 for reliable delivery
            
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                # print(f"MQTT Published: {json_payload}") # Uncomment for verbose logging
                self.last_published_state = current_state
            else:
                print(f"‼️ MQTT: Failed to publish. Error code: {result.rc}")

    # --- END NEW MQTT METHODS ---
    ...
    def load_icons(self):
        icon_mappings = {
            ...
            'emergency_alert': ['emergency_alert/emergency_alert.jpeg'], 
        }
        icon_size = (60, 60)
...
# ==================== MAIN RUN LOOP (FINAL) ======================
    ...
    def run(self):
        ...
        if self.mqtt_client:
            print(f"MQTT communication is ACTIVE, sending telemetry to {self.TB_TELEMETRY_TOPIC}.")
        else:
            print("MQTT communication is DISABLED (Check token/connection).")
        ...
        # Time tracking for smooth MQTT publishing
        last_mqtt_publish_time = time.time()
        MQTT_PUBLISH_INTERVAL = 0.5 # Publish check frequency
...
# ------------------------------------------------------------------
            ...
            # 7. ThingsBoard MQTT Publishing
            if self.mqtt_client and (current_time - last_mqtt_publish_time) >= 1.0/2: # Check twice per second
                self.publish_mqtt_telemetry()
